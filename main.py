...
def admin_only(f):
    ...
    def decorated_function(*args,**kwargs):
        if current_user.email != "admin@email.com" and current_user.email != "hsumyatshweyeeo00@gmail.com":
            return abort(403)
...
def login():
    ...
    if form.validate_on_submit():
        ...
        if not user:
            flash("You don't have account with this email, register first!")
            return redirect(url_for('login'))
        if not check_password_hash(user.password,password):
            flash("Your password incorrect!")
            return redirect(url_for('login'))
        login_user(user)
